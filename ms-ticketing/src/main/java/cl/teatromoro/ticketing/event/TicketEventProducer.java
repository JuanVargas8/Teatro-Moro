...
public class TicketEventProducer {
    ...
    private static final String TOPIC_BASE = "ticketing.ticket";
    ...
    public void sendCreated(TicketCreatedEvent event) {
        send(event, "created", event.getId().toString());
    }

    public void sendUpdated(TicketUpdateEvent event) {
        send(event, "updated", event.getId().toString());
    }

    public void sendDeleted(TicketDeleteEvent event) {
